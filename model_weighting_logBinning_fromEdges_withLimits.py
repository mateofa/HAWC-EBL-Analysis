...
path_to_weights_file = '/Users/mateo/Documents/HAWC/EBL/EBLmodel_likelihood/mrk501_gp_SSC_ECPL_EBL_loglike_Fixed_FullRange_12knots_1nuI_50nuI_100um/median_all'
...
for j in range(len(lamb)):
    ...
    max_Int_array.append(hist_array[j].GetBinCenter(max_bin))
    #print max_bin
    #First, lower bounds
    for l in range(0,max_bin):

        ratio =  hist_array[j].Integral(0,l)/hist_array[j].Integral()
        if (ratio > 0.16):
            lower_bound_68[j][l] = hist_array[j].GetBinCenter(l-1)
            lower_containment_68[j][l] = hist_array[j].Integral(0,l-1)/hist_array[j].Integral()

        if (ratio > 0.05):
            lower_bound_90[j][l] = hist_array[j].GetBinCenter(l-1)
            lower_containment_90[j][l] = hist_array[j].Integral(0,l-1)/hist_array[j].Integral()

        if (ratio > 0.025):
            lower_bound_95[j][l] = hist_array[j].GetBinCenter(l-1)
            lower_containment_95[j][l] = hist_array[j].Integral(0,l-1)/hist_array[j].Integral()

        print lower_containment_95
    ...
    if (len(np.trim_zeros(lower_bound_68[j]))>1):
        lower_bound_68_high = np.trim_zeros(lower_bound_68[j])[1]
        lower_bound_68_low= np.trim_zeros(lower_bound_68[j])[0]

        lower_containment_68_high = np.trim_zeros(lower_containment_68[j])[1]
        lower_containment_68_low = np.trim_zeros(lower_containment_68[j])[0]

        low_bound_68.append(np.interp(0.16,[lower_containment_68_low,lower_containment_68_high],[lower_bound_68_low,lower_bound_68_high]))
    ...
    if (len(np.trim_zeros(lower_bound_90[j]))>1):
        ...
        low_bound_90.append(np.interp(0.05,[lower_containment_90_low,lower_containment_90_high],[lower_bound_90_low,lower_bound_90_high]))
    ...
    if (len(np.trim_zeros(lower_bound_95[j]))>1):
        ...
        low_bound_95.append(np.interp(0.025,[lower_containment_95_low,lower_containment_95_high],[lower_bound_95_low,lower_bound_95_high]))
    ...
    ##################   NOW HIGHER BOUND #########################
    for l in reversed(range(max_bin,hist_array[j].GetSize())):
        ...
        if (ratio > 0.16):
            ...
            higher_containment_68[j][l] = hist_array[j].Integral(l+1,hist_array[j].GetSize())/hist_array[j].Integral()
        if (ratio > 0.05):
            higher_bound_90[j][l] = hist_array[j].GetBinCenter(l+1)
            higher_containment_90[j][l] = hist_array[j].Integral(l+1,hist_array[j].GetSize())/hist_array[j].Integral()

        if (ratio > 0.025):
            higher_bound_95[j][l] = hist_array[j].GetBinCenter(l+1)
            higher_containment_95[j][l] = hist_array[j].Integral(l+1,hist_array[j].GetSize())/hist_array[j].Integral()

        #print l," ",higher_containment_95
    higher_bound_68[j]=np.flip(higher_bound_68[j])
    higher_containment_68[j]=np.flip(higher_containment_68[j])

    higher_bound_90[j]=np.flip(higher_bound_90[j])
    higher_containment_90[j]=np.flip(higher_containment_90[j])


    higher_bound_95[j]=np.flip(higher_bound_95[j])
    higher_containment_95[j]=np.flip(higher_containment_95[j])
    #Extract first 2 non-zero values of each array and interpolate to get accurate 0.68, 0.90 and 0.95 bounds
    ...
    if (len(np.trim_zeros(higher_bound_68[j]))>1):
        ...
        high_bound_68.append(np.interp(0.16,[higher_containment_68_low,higher_containment_68_high],[higher_bound_68_low,higher_bound_68_high]))
    ...
    if (len(np.trim_zeros(higher_bound_90[j]))>1):
        ...
        high_bound_90.append(np.interp(0.05,[higher_containment_90_low,higher_containment_90_high],[higher_bound_90_low,higher_bound_90_high]))
    ...
    if (len(np.trim_zeros(higher_bound_95[j]))>1):
        ...
        high_bound_95.append(np.interp(0.025,[higher_containment_95_low,higher_containment_95_high],[higher_bound_95_low,higher_bound_95_high]))
...
namewithdir_results = os.path.join(path_to_weights_file,'UpperLower_Limits_FromMax.dat')
f_results=open(namewithdir_results, 'w+')

for i in range(len(lamb)):
    print lamb[i]," ",max_Int_array[i]," ",low_bound_68[i]," ",high_bound_68[i]," ",low_bound_95[i]," ",high_bound_95[i]
    print >> f_results, lamb[i]," ",max_Int_array[i]," ",low_bound_68[i]," ",high_bound_68[i]," ",low_bound_95[i]," ",high_bound_95[i]
...
plt.plot(lamb,max_Int_array,linestyle = 'None', marker='o', markerfacecolor='red', markersize=6, label = "Max. intensity")
...
output_plot = os.path.join(path_to_weights_file,'SED_limits_FromEdges.png')
...
hist_array[0].Draw('h')
legend.AddEntry(graph_array[0],"68% containment region")
output_plot_2 = os.path.join(path_to_weights_file,'histograms_fromEdges.png')

legend.AddEntry(graph_array[0],"68% containment region")
output_plot_3 = os.path.join(path_to_weights_file,'histograms_selection_fromEdges.png')
...
gPad.SetLogx()
hist_array[3].Draw('h')
graph_array[3].SetFillColorAlpha(2,0.1)
graph_array[3].SetFillStyle(3544)
...
gPad.SetLogx()
hist_array[4].Draw('same')
graph_array[4].SetFillColorAlpha(2,0.1)
graph_array[4].SetFillStyle(3544)
graph_array[4].Draw('f')
line_low_array[4].Draw('same')
line_high_array[4].Draw('same')
line_MaxBin_array[4].Draw('same')
