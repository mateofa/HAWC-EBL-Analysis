...
for j in range(len(lamb)):
	...
	max_bin_array.append(hist_array[j].GetXaxis().GetBinCenter(max_bin))
	#print max_bin
	#First, lower bounds
...
namewithdir_results = os.path.join(output_path,'UpperLower_Limits.dat')
...
plt.ylim([0, 100])
output_plot = os.path.join(output_path,'SED_limits_FromMax.png')
...
for i in range(len(lamb)):
	...
	legend.Draw('same')
c0c.SaveAs("/Users/mateo/Documents/HAWC/EBL/EBLmodel_likelihood/mrkCombined_gp_SSC_ECPL_EBL_loglike_Fixed_FullRange_12Knots_1nuI_50nuI_100um/histograms.png")
