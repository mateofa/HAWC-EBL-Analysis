...
namewithdir_best = os.path.join(path_to_weights_file_501,'BestModel_like.txt')
...
for i in range(len(lamb)):
	print lamb[i]," ",low_bound_68[i]," ",high_bound_68[i]," ",low_bound_95[i]," ",high_bound_95[i]
	print >> f_results, lamb[i]," ",low_bound_68[i]," ",high_bound_68[i]," ",low_bound_95[i]," ",high_bound_95[i]
...
for i in range(len(lamb)):
	...
	legend.Draw('same')
c0c.SaveAs("/Users/mateo/Documents/HAWC/EBL/EBLmodel_likelihood/mrkCombined_gp_SSC_ECPL_EBL_loglike_Fixed_FullRange_12Knots_1nuI_50nuI_100um/histograms_fromBest.png")
...
plt.plot(lamb_low,int_low,'^',color='aqua')
plt.plot(lamb_up,int_up,'v',color='lawngreen')


output_plot = os.path.join(output_path,'SED_limits_FromBest.png')
