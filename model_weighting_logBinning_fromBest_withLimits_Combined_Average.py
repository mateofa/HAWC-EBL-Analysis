...
Intensity_best_421=np.empty([len(lamb)])
Intensity_best_501=np.empty([len(lamb)])

## Create array with intensities for each model (first index) and wavelenght (second index)

#retrieve best model line
namewithdir_best_501 = os.path.join(path_to_weights_file_501,'BestModel_like.txt')
namewithdir_best_421 = os.path.join(path_to_weights_file_421,'BestModel_like.txt')

f_best_model_501=open(namewithdir_best_501, 'r')
f_best_model_421=open(namewithdir_best_421, 'r')

best_model_number_line_501 = f_best_model_501.readlines()
best_model_number_line_421 = f_best_model_421.readlines()

for x in best_model_number_line_501:
	best_model_number_501= int(x.split(' ')[11])
for x in best_model_number_line_421:
	best_model_number_421= int(x.split(' ')[11])


print "The 'chosen' for mrk421 model is: ", best_model_number_421
print "The 'chosen' for mrk501 model is: ", best_model_number_501
...
for x in d_modelIntensities:
	...
	for s in range(len(lamb)):
		I=float(x.split('  ')[s])
		Intensity[i][s]=I
		if(i==best_model_number_421-1):
			Intensity_best_421[s]=I
		if(i==best_model_number_501-1):
			Intensity_best_501[s]=I

#Combine intensities of both models_like
Intensity_best=np.empty([len(lamb)])

for s in range(len(lamb)):
	Intensity_best[s]=0.5*(Intensity_best_421[s]+Intensity_best_501[s])
	print Intensity_best_421[s]," ",Intensity_best_501[s]," ",Intensity_best[s]
...
for j in range(len(lamb)):
    ...
    #max_bin = hist_array[j].FindBin(Intensity_best[j])
	## Get max bin
	max_bin = hist_array[j].FindBin(Intensity_best[j])
	#print max_bin
	#First, lower bounds
	...
	if (len(np.trim_zeros(lower_bound_68[j]))>1):
		...
		low_bound_68.append(np.interp(0.34,[lower_containment_68_low,lower_containment_68_high],[lower_bound_68_low,lower_bound_68_high]))
	...
	if (len(np.trim_zeros(lower_bound_90[j]))>1):
		...
		low_bound_90.append(np.interp(0.45,[lower_containment_68_low,lower_containment_68_high],[lower_bound_68_low,lower_bound_68_high]))
	...
	if (len(np.trim_zeros(lower_bound_95[j]))>1):
		...
		low_bound_95.append(np.interp(0.475,[lower_containment_95_low,lower_containment_95_high],[lower_bound_95_low,lower_bound_95_high]))
	...
	hist_array[j].SetFillColorAlpha( 38, 0.8 )
...
namewithdir_results = os.path.join(output_path,'UpperLower_Limits_Average_LatexFormat.dat')
f_results=open(namewithdir_results, 'w+')

for i in range(len(lamb)):
	print round(lamb[i],2)," & ",Intensity_best[i]," & ",round(low_bound_68[i],2)," & ",round(high_bound_68[i],2)," & ",round(low_bound_95[i],2)," & ",round(high_bound_95[i],2), " \\\\"
	print >> f_results, round(lamb[i],2)," & ",round(Intensity_best[i],2)," & ",round(low_bound_68[i],2)," & ",high_bound_68[i]," & ",low_bound_95[i]," & ",high_bound_95[i], " \\\\"

namewithdir_results = os.path.join(output_path,'UpperLower_Limits_Average.dat')
f_results=open(namewithdir_results, 'w+')
...
plt.title("Mrk Combined - ECPL - median")


#ax.set_yscale('log')
#ax.set_xscale('log')

ax.fill_between(lamb_new, high_bound_95_spline, low_bound_95_spline, facecolor='darkred', alpha=1, label='Containment band 95% (HAWC)',interpolate=True)
...
ax.fill_between(lamb_new, high_bound_68_spline, low_bound_68_spline, facecolor='lightblue', alpha=1, label='Containment band 68% (HAWC)',interpolate=True)
...
eblModel_501 = PyEBL.ebl(namewithdir_int, best_model_number_501)
y_501 = eblModel_501.eblSED(x)
#plt.loglog(x,y_501,linewidth=2., color='red', label = "Best agreement 501")

eblModel_421 = PyEBL.ebl(namewithdir_int, best_model_number_421)
y_421 = eblModel_421.eblSED(x)
#plt.loglog(x,y_421,linewidth=2., color='red', label = "Best agreement 421")
...
#Retrieve HESS points and errors
f_HESS=open('/Users/mateo/Documents/HAWC/EBL/VERITAS_EBL/Data_HESS.csv', 'r')
lines_HESS=    f_HESS.readlines()
...
int_HESS_err_low=[]
int_HESS_err_high=[]
#int_HESS_err=np.empty((0,2),float)


line_n=0
for x in lines_HESS:
		lamb_HESS.append(float(x.split(' ')[0]))
		lamb_HESS_err_low.append(float(x.split(' ')[1]))
		lamb_HESS_err_high.append(float(x.split(' ')[2]))

		#lamb_HESS_err=np.append(lamb_HESS_err, np.array([[float(x.split(' ')[1]),float(x.split(' ')[2])]]), axis=0)

		int_HESS.append(float(x.split(' ')[3]))
		int_HESS_err_low.append(float(x.split(' ')[4]))
		int_HESS_err_high.append(float(x.split(' ')[5]))

		#int_HESS_err=np.append(lamb_HESS_err, np.array([[float(x.split(' ')[4]),float(x.split(' ')[5])]]), axis=0)

		line_n += 1
f_HESS.close()


int_HESS_err = np.array(list(zip(int_HESS_err_low,int_HESS_err_high))).T
...
f_both_MAGIC=open('/Users/mateo/Documents/HAWC/EBL/VERITAS_EBL/Data_MAGIC.csv', 'r')


lines_both_MAGIC=    f_both_MAGIC.readlines()
lamb_up_MAGIC_both_MAGIC=[]
int_up_MAGIC_both_MAGIC=[]
lamb_low_MAGIC_both_MAGIC=[]
int_low_MAGIC_both_MAGIC=[]
line_n=0
for x in lines_both_MAGIC:
    if (line_n % 2) == 0:
        if(0.1<float(x.split(' ')[0])<100):
                lamb_low_MAGIC_both_MAGIC.append(float(x.split(' ')[0]))
                int_low_MAGIC_both_MAGIC.append(float(x.split(' ')[1]))
    else:
        if(0.1<float(x.split(' ')[0])<100):
                   lamb_up_MAGIC_both_MAGIC.append(float(x.split(' ')[0]))
                   int_up_MAGIC_both_MAGIC.append(float(x.split(' ')[1]))
    line_n += 1
f_both_MAGIC.close()
...
plt.errorbar(lamb_HESS,int_HESS,yerr=int_HESS_err,fmt='.',ecolor='royalblue',label='HESS (2017)')
plt.hlines(int_HESS , lamb_HESS_err_low, lamb_HESS_err_high,colors='royalblue')
ax.fill_between(lamb_low_both,int_low_both , int_up_both, facecolor='green', alpha=0.5, label='VERITAS 95% (2019)',interpolate=True)
ax.fill_between(lamb_low_MAGIC_both_MAGIC,int_low_MAGIC_both_MAGIC , int_up_MAGIC_both_MAGIC, facecolor='gray', alpha=0.5, label='MAGIC (2019)',interpolate=True)

plt.plot(lamb,Intensity_best,linestyle = 'None', marker='o', markerfacecolor='black', markersize=6,label='Combined Intensity (HAWC)' )
legend = TLegend(0.1,0.8,0.4,0.9)
...
output_plot = os.path.join(output_path,'SED_limits_FromBest_Average_wTitle_ModColors.png')
...
for i in range(len(lamb)):
	...
	#legend.Draw('same')
output_plot_2 = os.path.join(output_path,"histograms_fromBest_Average_wTitle_ModColors.png")
